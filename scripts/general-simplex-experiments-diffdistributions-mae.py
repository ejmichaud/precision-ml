...
def cfg():
    REPEATS = 3
    TRAIN_POINTS = [4**n for n in range(3, 9)]
    N_TEST_POINTS = 10000
    TARGETS = {
        # 'x^2': {
        #     'dimension': 1,
...
@ex.automain
def run(REPEATS, TRAIN_POINTS, N_TEST_POINTS, TARGETS):
        ...
        for D in tqdm(TRAIN_POINTS, leave=False):
            ...
            if dimension == 1:
                ...
                for _ in tqdm(range(REPEATS), leave=False):
                    xs_test = np.random.uniform(low=ranges[0][0], high=ranges[0][1], size=(N_TEST_POINTS,))
                    ys_test = target_fn(xs_test)
                    preds = fit(xs_test)
                    valid_idx = ~np.isnan(preds)
                    mae_loss = np.mean(np.abs(preds[valid_idx] - ys_test[valid_idx])).item()
                    ex.info[eqn]['regular'][D].append(mae_loss)
                ...
                for _ in tqdm(range(REPEATS), leave=False):
                    ...
                    valid_idx = ~np.isnan(preds)
                    mae_loss = np.mean(np.abs(preds[valid_idx] - ys_test[valid_idx])).item()
                    ex.info[eqn]['random'][D].append(mae_loss) 
            else:
                ...
                for _ in tqdm(range(REPEATS), leave=False):
                    ...
                    xis_test = [np.random.uniform(low=ranges[i][0], high=ranges[i][1], size=(N_TEST_POINTS,)) for i in range(dimension)]
