...
# coding: utf-8
"""
The purpose of this script is to fit all the AI Feynman datasets using
both a single network and with boosting, so as to see whether boosting really
gives much of an advantage.
"""
...
def train_and_observe(i, width, depth, activation, data_dir, eqn, device, dtype, seed):
    ...
    data = np.loadtxt(Path(data_dir) / Path(eqn), dtype=np.float64)
    n, d = data.shape
    x_i = data[:, :d-1]
    ...
    for i in range(depth):
        if i == 0:
            layers.append(nn.Linear(d-1, width))
    ...
    with torch.no_grad():
        ex.info[eqn]['whole'] = mse_loss_fn_torch(mlp(x_i), y_i).item()

    f1 = nn.Sequential(
        nn.Linear(d-1, int(width/2)),
        nn.Tanh(),
    ...
    with torch.no_grad():
        ex.info[eqn]['primary'] = mse_loss_fn_torch(f1(x_i), y_i).item()
        ex.info[eqn]['boosted'] = mse_loss_fn_torch(f1(x_i) + (1/c1)*f2(x_i), y_i).item()


@ex.automain
def run(width, depth, activation, data_dir, device, dtype, seed):

    for i, file in tqdm(list(enumerate(Path(data_dir).iterdir()))):
        eqn = file.parts[-1]
        train_and_observe(i, width, depth, activation, data_dir, eqn, device, dtype, seed)
