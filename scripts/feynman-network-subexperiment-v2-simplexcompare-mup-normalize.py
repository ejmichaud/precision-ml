...
ex = Experiment("feynman-network-subexperiment-v2-simplexcompare-mup-normalize")
...
def run(eqn, width, depth, lr, activation, 
    ...
    for i in range(depth):
        if i == 0:
            layers.append(nn.Linear(dimension, dummy_width))
            layers.append(activation_fn())
        elif i == depth - 1:
            layers.append(MuReadout(dummy_width, 1))
        else:
            layers.append(nn.Linear(dummy_width, dummy_width))
            layers.append(activation_fn())
    base_mlp = nn.Sequential(*layers).to(device)
    ...
    for i in range(depth):
        if i == 0:
            ...
            layers.append(nn.Linear(dummy_width, dummy_width))
            layers.append(activation_fn())
    delta_mlp = nn.Sequential(*layers).to(device)
    ...
    for i in range(depth):
        if i == 0:
            ...
            layers.append(MuReadout(width, 1))
        else:
    ...
    set_base_shapes(mlp, base_mlp, delta=delta_mlp)

    loss_fn = nn.MSELoss()
    optim = MuAdam(mlp.parameters(), lr=lr)
